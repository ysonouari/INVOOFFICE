...
import { ensureHtml2Canvas, ensureJsPdf } from './pdf-dependencies.js';
...
function writePageOverlay(pdf, textElements, isRtl) {
...
export function buildPages(f, stage){
  const pages = [];
...
export async function resolveHeaderImage(src, company){
...
export const PDF_ENGINE_KEY = 'fb_pdf_engine';

export function getPdfEngine(){
  try {
    const v = localStorage.getItem(PDF_ENGINE_KEY);
    if (v === 'jspdf') return 'jspdf';
    return 'pdfkit';
  } catch (_) {
    return 'pdfkit';
  }
...
export function setPdfEngine(engine){
  try {
    if (engine === 'pdfkit' || engine === 'jspdf') {
      if (engine === 'pdfkit') localStorage.removeItem(PDF_ENGINE_KEY);
      else localStorage.setItem(PDF_ENGINE_KEY, 'jspdf');
    }
  } catch (_) {}
}

export async function renderPagesToPdf(payload, headerUrl){
  if (getPdfEngine() === 'pdfkit') {
    try {
      const engine = await import('./pdfkit-engine.js');
      return await engine.renderPagesToPdfPdfkit(payload, headerUrl);
    } catch (err) {
      console.error('[pdf] moteur pdfkit en échec → fallback jsPDF', err);
    }
  }
  ...
  for (let p = 0; p < pages.length; p++) {
    ...
    const canvas = await html2canvas(pages[p], { scale, useCORS: true, backgroundColor: header ? null : '#ffffff', logging: false });
