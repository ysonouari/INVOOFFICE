...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  window.addEventListener('keydown', e => { if (e.key === 'Escape') { closeAllModals(); setAdminMenuOpen(false); } });

  // Hamburger menu toggle (F2.4 — aria-expanded synchronisé + Escape)
  ...
  if (hamburger && adminNav) {
    hamburger.addEventListener('click', function() {
      setAdminMenuOpen(!adminNav.classList.contains('open'));
    });
