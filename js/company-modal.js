...
let onModalOverlayPointerDown = null;
let onHeaderToggleFocus = null;
...
if (uploadBoxEl) {
  uploadBoxEl.addEventListener('keydown', (e) => {
    if (e.key === 'Enter' || e.key === ' ') {
      e.preventDefault();
      const input = document.getElementById('headerFileInput');
...
export async function openCompanyModal(){
  ...
  document.getElementById('companyModalOverlay').addEventListener('pointerdown', onModalOverlayPointerDown = (e) => {
  ...
  document.getElementById('cHeaderActive').addEventListener('focus', onHeaderToggleFocus = () => {
    const md = document.querySelector('#companyModalOverlay .modal');
...
export function closeCompanyModal(){
  ...
  const overlay = document.getElementById('companyModalOverlay');
  if (onModalOverlayPointerDown && overlay) {
    overlay.removeEventListener('pointerdown', onModalOverlayPointerDown, true);
    onModalOverlayPointerDown = null;
  }
  if (onHeaderToggleFocus) {
    const toggle = document.getElementById('cHeaderActive');
    if (toggle) toggle.removeEventListener('focus', onHeaderToggleFocus);
    onHeaderToggleFocus = null;
  }
