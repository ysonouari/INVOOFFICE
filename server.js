...
const server = http.createServer((req, res) => {
  ...
  let resolved = path.normalize(path.join(ROOT, filePath));
  ...
  fs.readFile(resolved, (err, data) => {
    if (err) {
      if (err.code === 'ENOENT' || err.code === 'EISDIR' || err.code === 'EINVAL') {
        ...
        fs.readFile(indexFile, (err2, data2) => {
          if (err2) {
            // Serveur 404.html brandé (comportement Vercel statique)
            const notFound = path.join(ROOT, '404.html');
            fs.readFile(notFound, (err3, data3) => {
              if (err3) { res.writeHead(404); res.end('Not found'); return; }
              res.writeHead(404, { 'Content-Type': 'text/html; charset=utf-8', 'X-Content-Type-Options': 'nosniff', 'X-Frame-Options': 'DENY' });
              res.end(data3);
            });
            return;
          }
