...
test.describe('PDF — Pagination multi-pages', () => {
  test('footer sur chaque page, aucune ligne perdue/dupliquée, fond sur chaque page', async ({ page }) => {
    ...
    const result = await page.evaluate(async (redPng) => {
      ...
      // Scénario 1 : sans en-tête perso (fond par défaut entit), 60 lignes → plusieurs pages
      ...
      // Scénario 2 : avec en-tête perso (header actif), 45 lignes
      ...
      return { res1, pdf1Data, res2, pdf2Data };
    }, RED_PNG);

    // --- Scénario 1 : sans fond ---
    const numPages1 = (await countImagesPerPage(result.pdf1Data)).length;
    const numPages2 = (await countImagesPerPage(result.pdf2Data)).length;
    expect(numPages1).toBeGreaterThanOrEqual(3);
    ...
    expect(numPages2).toBeGreaterThanOrEqual(2);
    ...
    // --- Vérification des images par page (fond entit/header natif + contenu) ---
    const imgs1 = await countImagesPerPage(result.pdf1Data);
    const imgs2 = await countImagesPerPage(result.pdf2Data);
    expect(imgs1.length).toBe(numPages1);
    expect(imgs2.length).toBe(numPages2);
    for (const n of imgs1) expect(n).toBe(2); // fond natif entit + contenu PNG transparent
    for (const n of imgs2) expect(n).toBe(2); // en-tête natif + contenu
  });
});
