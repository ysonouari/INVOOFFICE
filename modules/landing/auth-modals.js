...
let modalsPrevFocus = null;

function showModal(name) {
  Object.values(modals).forEach(m => { if (m) m.style.display = 'none'; });
  if (modals[name]) {
    if (!modalsPrevFocus && document.activeElement && document.activeElement !== document.body) {
      modalsPrevFocus = document.activeElement;
    }
...
function hideAllModals() {
  Object.values(modals).forEach(m => { if (m) m.style.display = 'none'; });
  if (modalsPrevFocus && typeof modalsPrevFocus.focus === 'function') {
    try { modalsPrevFocus.focus(); } catch (_) {}
  }
  modalsPrevFocus = null;
}
