      ...
      <table class="admin-table">
        ...
        <tbody>
          ${logs.map(l => `<tr>
            ...
            <td style="font-size:12px;color:var(--muted);">${esc(l.admin_id?.slice(0,8) || '-')}</td>
            <td>${esc(actions[l.action] || l.action)}</td>
            <td>${esc(l.details?.target_name || l.target_user_id?.slice(0,8) || '-')}</td>
            <td style="color:var(--muted);font-size:12px;">${esc(l.details?.summary || '-')}</td>
          </tr>`).join('')}
        </tbody>
      </table>
    </div>`;
