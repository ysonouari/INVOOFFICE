...
import { confirmAction, showToast } from '../shared/ui.js';
        ...
        <tbody>
          ${allPayments.map(p => `
            <tr>
              ...
              <td style="font-size:12px;color:var(--muted);">${esc(p.reference || '-')}</td>
...
function renderPaymentView(p) {
  return `
    <div class="detail-grid">
      ...
      ${row('Devise', esc(p.currency || 'MAD'))}
      ${row('Méthode', esc(p.payment_method))}
      ${row('Référence', esc(p.reference || '-'))}
